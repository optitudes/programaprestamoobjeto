...
public class Empresa {
	...
	public Empresa(String nombre, String nit,int numeroClientes,int numeroEmpleados,
		...
		this.listaEmpleados=new Empleado[numeroEmpleados];
		this.listaPrestamos=new Prestamo[numeroPrestamos];
		this.listaObjetos  =new Objeto[numeroObjetos];
	...
	/**
	 * método que crea un cliente, si el cliente ya existe lo informa y si
	 * no hay cupo genera una exepción.
	 * @param documento
	 * @param nombre
	 * @param genero
     ...
     */
	public void crearEmpleado(String nombre, String codigo, String correo, String aniosExperiencia) throws EmpleadoExisteException
    {
		int posicion;
		boolean creado=false;
		...
		if(posicion==-1)
		{
			for(int i=0;i<listaEmpleados.length && !creado ;i++)
			{
				if(listaEmpleados[i]==null)
				{
					...
					creado=true;
					imprimirVentana("El empleado "+listaEmpleados[i].getNombre()+
									" ha sido creado con exito.");
				}
			}
		}
		...
		{
			throw new EmpleadoExisteException("El empleado con la identificación "+codigo
					                          +" ya existe en la posicion :"+ ++posicion);
		}
    }
	/**
	 ...
	 * @return
	 */
	private int obtenerPosicionEmpleado(String codigoBuscar) {
		...
		for (int i=0;i<listaEmpleados.length && !encontrado; i++) {
	...
	/**
     * método que imprime  los empleados registrados, los imprime enumerados con sus nombres y codigos
...
// punto 4
 ...
 /**
  * método que verifica si hay cupos disponibles para crear un empleado
  * si no los hay retorna una excepción, si los hay  no genera la excepción.
 * @throws SinCupoEmpleadoException 
  */
public void verificarCupoEmpleado() throws SinCupoEmpleadoException {
	boolean error=true;
	
	for(int i=0;i<listaEmpleados.length && error==true;i++) {
		if(listaEmpleados[i]==null)
			error=false;
	}
